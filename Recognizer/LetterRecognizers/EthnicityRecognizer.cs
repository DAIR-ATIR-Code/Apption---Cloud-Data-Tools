...
namespace RecognizerTools
{
    ...
    public class EthnicityRecognizer : Recognizer, ILetterRecognizer, IMediumStringRecognizer
    {
        private ColumnMetadata metadata;

        public EthnicityRecognizer(ColumnMetadata metadata)
        {
        ...
        public override bool IsMatch(string data, CancellationToken cancellationToken)
        {
            ...
            {
                if (ReferenceHelper.ReferenceDic["Ethnicities"].Contains(data))
                    return true;
            }
            return false;
        }
