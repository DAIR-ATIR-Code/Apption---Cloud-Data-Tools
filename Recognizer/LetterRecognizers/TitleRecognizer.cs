...
namespace RecognizerTools
{
    ...
    public class TitleRecognizer : Recognizer, ILetterRecognizer, IShortStringRecognizer
    {
        private ColumnMetadata metadata;

        private const int MAX_LENGTH = 10;
        public TitleRecognizer(ColumnMetadata metadata)
        {
        ...
        public override bool IsMatch(string data, CancellationToken cancellationToken)
        {
            ...
            if (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (ReferenceHelper.ReferenceDic["PersonTitle"].Contains(data))
                    return true;
            }
            return false;
        }
