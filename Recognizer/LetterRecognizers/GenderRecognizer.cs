...
namespace RecognizerTools
{
    ...
    public class GenderRecognizer : Recognizer, ILetterRecognizer, IShortStringRecognizer, IMediumStringRecognizer
    {
        private ColumnMetadata metadata;


        private const int MAX_GENDER_LENGTH = 10;

        public GenderRecognizer(ColumnMetadata metadata)
        {
        ...
        public override bool IsMatch(string data, CancellationToken cancellationToken)
        {
            ...
            {
                if (ReferenceHelper.ReferenceDic["Gender"].Contains(data))
                    return true;
            }
            return false;
        }
